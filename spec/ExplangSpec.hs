...
spec = do
  describe "parseExpectation" $ do
    ...
    test "intransitively calls" (Expectation intransitiveFlag Unscoped (Inspection "calls" Any Unmatching) AnyCount)
    test "intransitively calls `foo`" (Expectation intransitiveFlag Unscoped (Inspection "calls" (Named "foo") Unmatching) AnyCount)
    test "intransitively calls something like `foo`" (Expectation intransitiveFlag Unscoped (Inspection "calls" (Like "foo") Unmatching) AnyCount)
    test "intransitively calls something distinct of `foo`" (Expectation intransitiveFlag Unscoped (Inspection "calls" (Except "foo") Unmatching) AnyCount)
    test "intransitively calls any of (`foo`, `bar`, `baz`)" (Expectation intransitiveFlag Unscoped (Inspection "calls" (AnyOf ["foo", "bar", "baz"]) Unmatching) AnyCount)

    test "intransitively declares class like `Foo`" (Expectation intransitiveFlag Unscoped (Inspection "declares class" (Like "Foo") Unmatching) AnyCount)
    test "intransitively declares class distinct of `Foo`" (Expectation intransitiveFlag Unscoped (Inspection "declares class" (Except "Foo") Unmatching) AnyCount)
    test "intransitively declares method like `foo`" (Expectation intransitiveFlag Unscoped (Inspection "declares method" (Like "foo") Unmatching) AnyCount)
    test "intransitively declares method distinct of `foo`" (Expectation intransitiveFlag Unscoped (Inspection "declares method" (Except "foo") Unmatching) AnyCount)
    ...
    test "within `bar` calls exactly 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" Any Unmatching) (Exactly 3))
    test "within `bar` calls `foo` exactly 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Named "foo") Unmatching) (Exactly 3))
    test "within `bar` calls something like `foo` exactly 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Like "foo") Unmatching) (Exactly 3))
    test "within `bar` calls something distinct of `foo` exactly 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Except "foo") Unmatching) (Exactly 3))
    test "within `bar` calls any of (`foo`, `bar`, `baz`) exactly 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (AnyOf ["foo", "bar", "baz"]) Unmatching) (Exactly 3))
    ...
    test "within `bar` calls at least 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" Any Unmatching) (AtLeast 3))
    test "within `bar` calls `foo` at least 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Named "foo") Unmatching) (AtLeast 3))
    test "within `bar` calls something like `foo` at least 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Like "foo") Unmatching) (AtLeast 3))
    test "within `bar` calls something distinct of `foo` at least 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Except "foo") Unmatching) (AtLeast 3))
    test "within `bar` calls any of (`foo`, `bar`, `baz`) at least 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (AnyOf ["foo", "bar", "baz"]) Unmatching) (AtLeast 3))

    test "within `bar` calls at most 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" Any Unmatching) (AtMost 3))
    test "within `bar` calls `foo` at most 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Named "foo") Unmatching) (AtMost 3))
    test "within `bar` calls something like `foo` at most 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Like "foo") Unmatching) (AtMost 3))
    test "within `bar` calls something distinct of `foo` at most 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (Except "foo") Unmatching) (AtMost 3))
    test "within `bar` calls any of (`foo`, `bar`, `baz`) at most 3 times" (Expectation noFlags (Scoped "bar") (Inspection "calls" (AnyOf ["foo", "bar", "baz"]) Unmatching) (AtMost 3))
